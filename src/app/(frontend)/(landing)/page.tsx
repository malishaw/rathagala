...
import { FeaturedDealers } from "@/features/organizations/components/featured-dealers";
...
export default function VehicleMarketplace() {
  ...
  useEffect(() => {
    if (data?.ads && data.ads.length >= 0) {
      if (currentPage === 1) {
        // Reset to first page results (even if empty), but check for equality to prevent double-renders
        setAllAds((prevAds) => {
          if (
            prevAds.length === data.ads.length &&
            prevAds.every((ad, i) => ad.id === data.ads[i].id)
          ) {
            return prevAds;
          }
          return data.ads;
