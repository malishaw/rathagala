...
/**
 * Fetches an object from Cloudflare R2 via S3 API using pure web fetch and AWS SigV4.
 */
export async function getObjectR2({
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "");
    throw new Error(`Cloudflare R2 fetch failed (${response.status} ${response.statusText}): ${errorText}`);
  }
